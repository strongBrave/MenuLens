...
export default function DetailPanel({ dish }) {
  ...
  return (
    <div className="flex-1 h-full bg-white flex flex-col animate-fade-in overflow-y-auto">
      ...
      {isZoomed && (
        ...
        >
          <div className="relative w-full h-full flex flex-col items-center justify-center">
             ...
             />
             <button 
               className="absolute top-4 right-4 text-white hover:text-gray-300 bg-white/20 hover:bg-white/30 p-3 rounded-full transition-all transform hover:scale-110 backdrop-blur-sm border border-white/30 shadow-lg"
               onClick={(e) => {
                 e.stopPropagation();
                 setIsZoomed(false);
               }}
             ...
             <div className="absolute bottom-8 text-white/90 text-sm font-bold bg-black/60 px-6 py-3 rounded-full backdrop-blur-md border border-white/10 shadow-xl">
      ...
      <div 
        className="relative w-full h-[45vh] min-h-[300px] shrink-0 bg-gray-900 group cursor-zoom-in overflow-hidden"
      ...
      >
        <div className="absolute inset-0 flex items-center justify-center bg-gray-900">
          ...
          {/* Main Image - Contain Mode to show full image */}
          <img 
            src={getCurrentImageUrl()} 
            alt={dish.english_name} 
            className="relative h-full w-full object-contain z-10 transition-transform duration-500 group-hover:scale-[1.02]"
          />
        </div>
        ...
        <div className="absolute bottom-6 right-6 flex items-center gap-3 z-20" onClick={e => e.stopPropagation()}>
           {/* Counter */}
           {dish.image_urls && dish.image_urls.length > 1 && (
             <div className="bg-black/70 text-white px-3 py-1.5 rounded-full text-xs font-bold tracking-wider border border-white/10 shadow-lg cursor-default">
           ...
           {dish.image_urls && dish.image_urls.length > 1 && (
              <button
                ...
                className="bg-white text-indigo-600 hover:bg-indigo-50 p-2.5 rounded-full shadow-xl transition-all active:scale-95 border-2 border-indigo-100 cursor-pointer pointer-events-auto"
                title="Next Image"
              >
