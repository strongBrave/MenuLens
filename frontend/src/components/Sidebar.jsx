...
export default function Sidebar({ onUpload, isLoading, onReset, hasResults }) {
  ...
  return (
    <aside className="w-full md:w-[400px] lg:w-[450px] bg-cream-50 border-r border-slate-200 flex flex-col h-full flex-shrink-0 relative z-20 shadow-xl">
      {/* Brand Header */}
      <div className="p-8 pb-4">
        <div className="flex items-center gap-3 mb-1 cursor-pointer group" onClick={handleLocalReset}>
           <div className="w-10 h-10 bg-brand-600 rounded-xl flex items-center justify-center text-white text-xl font-bold shadow-brand-200 shadow-lg group-hover:scale-110 transition-transform duration-300">
             M
           </div>
           <h1 className="text-2xl font-bold text-slate-800 tracking-tight group-hover:text-brand-600 transition-colors">MenuLens</h1>
        </div>
      ...
      <div className="flex-1 px-8 py-6 flex flex-col justify-center">
        ...
        <div 
           className={`relative overflow-hidden rounded-2xl border-2 transition-all duration-300 group h-[300px] flex flex-col items-center justify-center
             ${previewUrl ? 'border-transparent bg-slate-900 shadow-xl' : (dragActive ? 'border-brand-500 bg-brand-50/50 scale-[1.02] border-dashed' : 'border-slate-300 border-dashed bg-white hover:border-brand-400 hover:shadow-md')}
        ...
        >
           ...
           {previewUrl ? (
             ...
             <div className="p-10 text-center relative z-10">
                {/* Animated Icon */}
                <div className="w-20 h-20 bg-brand-50 text-brand-600 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-sm">
                ...
                <button 
                    className="w-full px-6 py-4 bg-gradient-to-r from-brand-600 to-purple-600 text-white rounded-xl text-base font-bold shadow-lg shadow-brand-200 hover:shadow-xl hover:shadow-brand-300 hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2 group-hover:from-brand-700 group-hover:to-purple-700"
                >
        ...
        {!hasResults && !previewUrl && (
          ...
          <div className="mt-12 space-y-6 px-2">
             <div className="flex items-center gap-4 group">
               <span className="flex-shrink-0 w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 font-bold text-sm group-hover:bg-brand-100 group-hover:text-brand-600 transition-colors">1</span>
               <div>
             ...
             <div className="flex items-center gap-4 group">
               <span className="flex-shrink-0 w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 font-bold text-sm group-hover:bg-brand-100 group-hover:text-brand-600 transition-colors">2</span>
               <div>
             ...
             <div className="h-4 w-0.5 bg-slate-200 ml-4 -my-2"></div>
             <div className="flex items-center gap-4 group">
               <span className="flex-shrink-0 w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 font-bold text-sm group-hover:bg-brand-100 group-hover:text-brand-600 transition-colors">3</span>
               <div>
      ...
      {/* Footer */}
      <div className="p-6 border-t border-slate-200 bg-cream-50/50">
